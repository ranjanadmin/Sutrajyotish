...
from engine.career_source_engine import (
    get_career_source
)
...
def api_prediction_career():
    ...
    try:
       ...
       prediction.update({
            ...
            "income": timeline.get("income", ""),
            "timeline": timeline.get("timeline", ""),
            "source": source_info.get("source", []),
            "position": source_info.get("position", []),
            "maha_strength": (
                maha_info.get("strength", "")
                if isinstance(maha_info, dict)
                else ""
            ),
            "bhukti_strength": (
                bhukti_info.get("strength", "")
                if isinstance(bhukti_info, dict)
                else ""
            ),
            "antara_strength": (
                antara_info.get("strength", "")
                if isinstance(antara_info, dict)
                else ""
            )
        })
