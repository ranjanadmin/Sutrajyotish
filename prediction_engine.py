...
CAREER_NEGATIVE = {8, 12}

def get_nature(houses):

    houses = set(houses)

    pos = len(houses & CAREER_POSITIVE)
    neg = len(houses & CAREER_NEGATIVE)
...
def score_row(row):
    ...
    planet_nature = get_nature(planet_houses)
    star_nature = get_nature(star_houses)
    sub_nature = get_nature(sub_houses)

    planet_weight = 1

    if star_nature == sub_nature:
        star_weight = 2
        sub_weight = 3
    else:
        star_weight = 2
    ...
    if (
        planet_nature == "POSITIVE"
        and star_nature == "POSITIVE"
        and sub_nature == "POSITIVE"
    ):
        vertical_bonus += 150

    elif (
        star_nature == "POSITIVE"
        and sub_nature == "POSITIVE"
    ):
        vertical_bonus += 100
        ...
        star_nature == "POSITIVE"
        and sub_nature == "NEGATIVE"
    ):
        vertical_bonus -= 100

    elif (
        star_nature == "NEGATIVE"
        and sub_nature == "NEGATIVE"
    ):
        vertical_bonus -= 150

    total += vertical_bonus
...
def predict_career_horary(kp_grid):
    ...
    for event in set(events):
        ...
        if event in NEGATIVE_EVENTS:
            negative_hits += 1
# Critical Career Event Overrides

    if "TERMINATION" in events:
       total -= 1000

    if "RESIGNATION" in events:
       total -= 700

    if "VALUE_DESTRUCTION" in events:
       total -= 500

    if "STRESSED_SERVICE" in events:
       total -= 300

    if "ADVERSE_WORK_ENVIRONMENT" in events:
       total -= 200
    # ----------------------------------
    # Hit Validation
    # ----------------------------------
